...
// Работа с 1 инстаграм аккаунтом
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    if (longLivedData?.access_token) {
      const instagramToken = longLivedData as IInstagramAccessToken;
      await setCookie({
    ...
    return NextResponse.json(longLivedData); // Возвращает долгоживущий токен
  } catch (error: any) {
